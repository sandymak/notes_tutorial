...
function calcDroneMinEnergy(path) {
  let minimalAmount = 0,
      currentTank = 0;

  for (let i = 0; i <= path.length - 1; i++) {
    let currentHeight = path[i][2],
        nextHeight = path[i + 1] ? path[i + 1][2] : undefined,
        difference = currentHeight - nextHeight;
    ...
    if (currentHeight > nextHeight) {
      ...
      minimalAmount = currentTank < minimalAmount ? currentTank : minimalAmount
  }

  return Math.abs(minimalAmount);
}
